...
// Adapted from the ReactBits source: the original tracks pointer events on the
// component's own container (meant to wrap page content). Here the cursor is
// mounted as a non-blocking fixed overlay (matches the previous FluidCursor
// pattern), so pointer tracking is switched to `window` and the container is
...
const GlowCursor = ({
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      const shouldFade = config.idleFade && idleFor > config.idleTimeout;
